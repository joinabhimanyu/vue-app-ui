...
class BaseProxy {
  ...
  submit(requestType, url, data = null) {
    ...
    return new Promise((resolve, reject) => {
      Vue.$http[requestType](url + self.getParameterString(), data)
        .then((response) => {
          resolve(response);
        })
        .catch(({ response }) => {
  ...
  /**
   * Method used to update multiple items
   * @param {any} items
   */
  updateList(items) {
    return this.submit('put', `/${this.endpoint}`, items);
  }
  /**
  ...
  /**
   * Method used to destroy list of items
   * @param {any} ids
   */
  destroyList(data) {
    return this.submit('delete', `/${this.endpoint}`, data);
  }
